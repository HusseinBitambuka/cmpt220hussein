public class Complex extends Number implements Cloneable, Comparable<Complex> {
    ...
    public double abs() {// a static method for absolute value or modulus of a complex number
        return Math.sqrt(getRealPart() * getRealPart() + getImaginaryPart() * getImaginaryPart()); // sqrt(real**2+imaginary**2)
    }

    public Complex divide(Complex a) { // division between two complex number
        double k = (this.a * a.getRealPart() + this.b * a.getImaginaryPart()) / Math.pow(a.abs(), 2);// the real part
        double h = (this.b * a.getRealPart() - this.a * a.getImaginaryPart()) / Math.pow(a.abs(), 2);// the imaginary
                                                                                                    // part
        return new Complex(k, h);
    }

    @Override
    // overidding the toString method to return a standard representation of complex
    // number
    ...
    @Override
    // overriding the compareTo method in the Comparable interface to compare two
    // complex numbers
    public int compareTo(Complex a) {
        if (this.a > a.getRealPart()) {
            // check if the real part our complex is bigger than the other real part complex
            // and return one if it is the case
            return 1;
        } else if (this.a == a.getRealPart()) {
            // check if the real parts of the two complex numbers are equal, and if they
            // are:
            ...
            // check if the real part our complex is less than the real part of the other
            // complex and return negative one if it is the case
            return -1;
        }
    }
